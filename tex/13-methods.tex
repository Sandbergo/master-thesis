...
\textit{\gls{MLP}3} is illustrated in \Cref{fig:topo_mlp3}. The second ablation removes the convolutions in its entirety, resulting in an \gls{MLP} model consisting of the prenorm layer and embedding of the variable features with the output module. This equates to a four layer \gls{MLP}
...
The theoretical basis for \gls{Ecole} is presented in \Cref{sec:mdp}, with the \gls{PO-MDP} formulation relevant for the  
